...
while len(correct_guess) < 50:
    answer_states = screen.textinput(title=f"{len(correct_guess)}/50 States Correct", prompt="Guess another U.S States ? ").title()
...
for state_data in us_state_list:
    if state_data not in correct_guess:
        missing_state.append(state_data)
